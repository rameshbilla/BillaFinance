...
@Component({
  ...
  template: `
    <div class="fixed inset-0 z-[150] flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-md px-0 sm:px-4 overflow-hidden" (click)="onBackdrop($event)">
      <div class="bg-white dark:bg-gray-900 w-full sm:max-w-md rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl animate-scale-in flex flex-col max-h-[90vh] overflow-hidden" (click)="$event.stopPropagation()">
        ...
        <!-- Header -->
        <div class="px-6 pt-4 pb-3 border-b border-gray-50 dark:border-gray-800 shrink-0">
        ...
        <!-- Scrollable content -->
        <div class="flex-1 overflow-y-auto custom-scrollbar space-y-4 py-4">
          <!-- Bill Amount Reference & History -->
          <div class="mx-6 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-2xl border border-amber-100 dark:border-amber-800/30 flex justify-between items-center">
          ...
          <div *ngIf="bill?.payments && bill!.payments!.length > 0" class="mx-6 max-h-32 overflow-y-auto space-y-2 no-scrollbar border-b border-gray-100 dark:border-gray-800 pb-3">
          ...
          <!-- Form -->
          <form [formGroup]="payForm" (ngSubmit)="onSubmit()" class="px-6 space-y-4">
